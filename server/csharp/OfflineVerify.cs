...
namespace SafetyNetCheck
{
    ...
    public static class OfflineVerify
    {
        ...
        public static AttestationStatement ParseAndVerify(string signedAttestationStatement)
        {
            ...
            {
                // The certificate isn't issued for the hostname attest.android.com.
                return null;
            }
        ...
        /// <summary>
        /// Verifes an X509Security key, and checks that it is issued for a given hostname.
        /// </summary>
        /// <param name="hostname">The hostname to check to.</param>
        /// <param name="securityKey">The security key to verify.</param>
        /// <returns>true if securityKey is valid and is issued to the given hostname.</returns>
        ...
        /// </summary>
        /// <param name="token">The token where the keys are to be retrieved from.</param>
        /// <returns>The embedded security keys. null if there are no keys in the security token.</returns>
        /// <exception cref="KeyNotFoundException">Thrown when the JWT data does not contain a valid signature
        /// header "x5c".</exception>
        /// <exception cref="CryptographicException">Thrwon when the JWT data does not contain valid signing
        /// keys.</exception>
        private static X509SecurityKey[] GetEmbeddedKeys(JwtSecurityToken token)
        {
            // The certificates are embedded in the "x5c" part of the header. We extract them, parse them,
            // and then create X509 keys out of them.
